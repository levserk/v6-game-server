...
ChatManager.prototype.onMessage = function(message, type){
    ...
    switch (type){
        case 'message':
            ...
            break;
        case 'load':
            this.loadMessages(message.sender, data.count, data.time, data.target || this.default, data.type);
            break;
        case 'ban':
...
ChatManager.prototype.loadMessages = function (user, count, time, target, type){
    count = +count;
    time = +time;
    ...
    this.server.storage.getMessages(count, time, target, type ? null : user.userId)
