...
function GameManager(server){
    ...
    server.on('user_leave', function(user){
        var room = self.getUserRoom(user, true);    // get room where user is player
        if (room && (!room.isPlaying() || server.conf.loseOnLeave)) {
            self.onUserLeave(user.currentRoom, user);
        }
        if (!room){
            room = self.getSpectatorRoom(user);   // get room where user is spectator
            if (room){
                self.onSpectatorLeave(room, user);
            }
        }
    });
        ...
        if (room) {
            ...
            server.router.send({
                ...
                data:{
                    ...
                    nextPlayer: room.game.current.userId,
                    userTime: room.timeout ? Date.now() - room.game.turnTime : null,
                    playerTurns: room.game.playerTurns
                }
            });
            room.game.askDraw = null;
            if (!server.conf.reconnectOldGame || !room.isPlaying())
    ...
    server.inviteManager.on('invite_accepted', function(invite){
        ...
        for (var i = 0; i < invite.players.length; i++){
            ...
            if (self.getSpectatorRoom(player)){
                self.onSpectatorLeave(player.currentRoom, player);
            }
        }
...
GameManager.prototype.onUserSpectate = function(room, user){
    ...
    this.server.router.send({
        ...
        data:{
            ...
            history: room.game.shistory,
            nextPlayer: room.game.current.userId,
            userTime: room.game.timeout ? Date.now() - room.game.turnTime : null,
            playerTurns: room.game.playerTurns
        }
    });
