...
function GameManager(server){
    ...
    server.inviteManager.on('invite_accepted', function(invite){
        util.log('log;', 'invite_accepted', invite);
    });
}
...
GameManager.onMessage = function(message, type){
    var room;
...
GameManager.prototype.createGame = function(owner, players, data){
    ...
    this.server.router.send({
        module: 'server',
        type: 'new_game',
        target: {room: server.game},
        data: {players:players, room:room.id}
    });
...
GameManager.prototype.endGame = function(room) {
    ...
    this.server.router.send({
        ...
        type: 'end_game',
        target: {room: server.game},
        data: {players:room.players, roomId:room.id}
    });
};
