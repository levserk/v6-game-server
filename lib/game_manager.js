...
GameManager.prototype.onUserTurn = function(room, user, turn){
    ...
    // switch player
    var nextPlayer;
    if (typeof this.engine.switchPlayer == "function") nextPlayer = this.engine.switchPlayer(room, user, turn) || user;
    else {
    ...
    if (nextPlayer != game.current ) this.savePlayerTurn(room);
    // send turn
    this.server.router.send({
        module: 'game_manager',
        type: 'turn',
