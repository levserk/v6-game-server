...
GameManager.prototype.onUserTurn = function(room, user, turn){
    ...
    // switch player
    var nextPlayer;
    if (typeof this.engine.swithPlayer == "function") nextPlayer = this.engine.swithPlayer(room, user, turn);
    else {
    ...
    this.server.router.send({
        ...
        type: 'turn',
        target: {room: room.id},
        data: {user:user.id, turn:userTurn}
    });

    // check endGame
    var winner = false; // false - game not end, null - draw, user - winner
    if (typeof this.engine.checkWinner == "function") winner = this.engine.checkWinner(room, user, turn);
    else if (turn == 'win') winner = user;
    if (winner !== false) { // game end
        this.onRoundEnd(room, winner);
        return;
    }
    if (nextPlayer != room.game.current){// switch
...
GameManager.prototype.onUserLeave = function(room, user){
    ...
    for (i = 0; i < room.players.length; i++)
        if (room.players[i] != user) {
            this.onRoundEnd(room, room.players[i]);
            break;
        }
...
GameManager.prototype.onRoundEnd = function(room, winner){
    util.log('log;', 'GameManager.onRoundEnd, room:', room.id, 'winner:', winner?winner.userId:null);
    // TODO: save results
    var result = {
