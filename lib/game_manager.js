...
GameManager.prototype.onRoundEnd = function(room, result){
    ...
    util.log('log;', 'GameManager.onRoundEnd, room:', room.id, 'result and action:', result.winner, result.action, 'save: ',result.save);
    room.game.state = "waiting";
    if (this.server.conf.mode == 'test') {
        result.history = room.game.history;
        result.shistory = room.game.shistory;
    }

    if (result.save) {
        this.server.ratingManager.computeNewRatings(room, result);
        room.games++;
...
GameManager.prototype.saveGame = function(room, result){
    ...
    util.log('log;', 'GameManager.saveGame, save: ', save, 'time: ', Date.now() - save.timeEnd);
