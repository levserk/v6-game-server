...
GameManager.prototype.createRoom = function(owner, players, data){
    ...
    room.resetTimerEveryTurn = this.server.conf.resetTimerEveryTurn || undefined;
...
GameManager.prototype.onUserTurn = function(room, user, turn){
    ...
    if (turn.action == 'timeout') {
        logger.err('GameManager.onUserTurn, timeout received, wrong turn: ', turn, 1);
        this.sendError(user, 'wrong turn');
        return;
    }
    ...
    // switch player
    nextPlayer = this.switchPlayer(room, user, turn);
    ...
    // check endGame
    isGameEnd = this.checkGameEnd(room, user, turn);
    ...
    if (!isGameEnd && (nextPlayer != game.current || this.server.conf.resetTimerEveryTurn)) {
        if (room.timeout && this.server.conf.resetTimerEveryTurn) clearTimeout(room.timeout);
...
GameManager.prototype.doTakeBack = function(room, user) {
    var game = room.game;
    room.askTakeBack = null;
    if (user == game.current && game.playerTurns.length>0){
        game.playerTurns = [];
...
GameManager.prototype.onTimeout = function(room, user){
    console.log(user.userId, room.data[user.userId].timeouts);
    ...
    if (typeof this.engine.doTurn == 'function'){
        turn = this.engine.doTurn(room, user, turn);
    } else {
        turn = this.defaultEngine.doTurn(room, user, turn);
    }
    nextPlayer = this.switchPlayer(room, user, 'timeout');
    ...
    isGameEnd = this.checkGameEnd(room, user, 'timeout');
...
GameManager.prototype.onRoundEnd = function(room, result, callback){
    ...
    result.save = result.save || room.game.turns - room.game.timeouts >= this.minTurns;
