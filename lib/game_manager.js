...
function GameManager(server){
    ...
    server.on('user_leave', function(user){
        var room = self.getUserRoom(user, true);
        ...
        if (room) {
            ...
            server.router.send({
                ...
                data:{
                    ...
                    userTime: room.game.turnTime ? Date.now() - room.game.turnTime : null,
...
GameManager.prototype.setUserReady = function(room, user, ready){
    ...
    if (room.checkPlayersReady()){ // all users ready
        ...
        if (typeof this.engine.gameEvent == "function") this.sendEvent(this.engine.gameEvent(room, null, null, true));
    }
};
...
GameManager.prototype.onUserTurn = function(room, user, turn){
    ...
    if (!this.checkGameEnd(room, user, turn) && nextPlayer != game.current) {// switch and check event
        ...
        if (typeof this.engine.gameEvent == "function")                     // send event
            this.sendEvent(this.engine.gameEvent(room, user, turn, false));
    ...
    switch (event.type){
        ...
        default:
            ...
            if (typeof this.engine.userEvent == "function"){
                this.sendEvent(this.engine.userEvent(room, user, event));
            }
    }
};


GameManager.prototype.sendEvent = function(data) {
    if (!data) return;

    if (data.length > 0) { // array of users and their events
        for (var i = 0; i < data.length; i++)
            this.sendEvent(data[i]);
        return;
    }
