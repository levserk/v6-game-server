...
GameManager.prototype.onUserTurn = function(room, user, turn){
    ...
    // switch player
    nextPlayer = this.switchPlayer(room, user, turn, 'turn');
    userTurn.userTurnTime = room.getTurnTime(nextPlayer);
...
GameManager.prototype.updateTime = function(room, nextPlayer){
    ...
    if (room.timeMode == 'dont_reset') {
        ...
        // TODO: check if user time is out
    }
...
GameManager.prototype.onRoundEnd = function(room, result, callback){
    ...
    result.save = result.save != null ? result.save : room.game.turns - room.game.timeouts >= this.minTurns;
    result.action = result.save != null ? result.action : 'game_over';
...
GameManager.prototype.saveGame = function(room, result){
    ...
    save.timeStart = result.timeStart;
    save.timeEnd = result.timeEnd;
    save.time = result.time;
