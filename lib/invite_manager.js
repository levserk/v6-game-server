...
InviteManager.prototype.onPlayRandom = function(user, data){
    ...
    // remove and check turn off
    if (this.removeWaitingUser(user) == data.mode || data == 'off'){
        return;
    }
...
InviteManager.prototype.removeWaitingUser = function(user){
    for (var i = 0; i < this.server.modes.length; i++){
        if (this.waiting[this.server.modes[i]] == user){
            ...
            var sendData = {};
            sendData[this.server.modes[i]] = null;
            this.server.router.send({
                module : "invite_manager",
                type: "random_cancel",
                target: this.server.game,
...
InviteManager.prototype.getWaitingUsers = function(){
    var waiting = {};
    for (var i = 0; i < this.server.modes.length; i++){
        if (this.waiting[this.server.modes[i]]){
            waiting[this.server.modes[i]] = this.waiting[this.server.modes[i]].userId;
        }
    }
    return waiting;
};