...
InviteManager.prototype.onInviteAccepted = function(invite){
    ...
    if (!this.invites[target.userId]){
        logger.log('InviteManager.onInviteAccepted', 'invite not exists, invite sender and accepted:', target.userId, invite.sender.userId, 1);
    }
...
InviteManager.prototype.onPlayRandom = function(user, data){
    ...
    if (this.removeWaitingUser(user) == data.mode || data == 'off'){ // remove and check switch off
        return;
    }
