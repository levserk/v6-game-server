...
function InviteManager(server){
    ...
    this.server.on("user_leave", function(user){
        ...
        if (self.waiting == user) self.waiting = null;
    });
}
...
InviteManager.prototype.onInvite = function(invite){
    ...
    if (this.server.gameManager.getUserRoom(invite.sender) || this.server.gameManager.getUserRoom(target)){
        util.log('warn;', 'InviteManager', 'invite user already in room!');
        return;
    }
...
InviteManager.prototype.onPlayRandom = function(user, data){
    util.log('log;', 'InviteManager.onRandomPlay', user.userId, data);
    if (!data && this.waiting == user){
        this.waiting = null;
        return;
    }
    if (this.server.gameManager.getUserRoom(user)){ //TODO: refactor this in metod
