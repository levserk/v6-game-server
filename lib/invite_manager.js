...
InviteManager.prototype.onInviteCancel = function(invite){
    ...
    if (!target) {
        logger.warn('InviteManager.onInviteCancel', 'no user', invite.target, 1);
        return;
    }
    this.server.router.send({
        module : "invite_manager",
...
InviteManager.prototype.onInviteAccepted = function(invite){
    ...
    if (this.server.gameManager.getUserRoom(invite.sender)){
        logger.log('InviteManager.onInviteAccepted', 'accepted user already in room', invite.sender.userId, 1);
        this.server.router.send({
            module : "invite_manager",
            type: "reject",
            sender: target,
    ...
    if (this.server.gameManager.getUserRoom(target)){
        logger.log('InviteManager.onInviteAccepted', 'sent user already in room', target.userId, 1);
        return;
    }
