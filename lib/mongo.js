            ...
            if (resultInsert.insertedCount == 1) {
                col.findOne({time: key}, function (err2, resultFind) {
                    ...
                    self.createIndexNotExists('test', 'time', {time: -1}).then(function(index) {
                        util.log('log;', 'Mongo.init test index result: ', index == 'time' ? 'complete' : 'failed');
        ...
        self.connect()
            .then(function (db){
                ...
                col.find(query, { "sort": [['time','desc']], limit:count}).toArray(function (err, items){
                    //util.log('log;', 'Mongo.loadRanks query: db.messages.find(', query, ').sort({time: -1}) time: ', Date.now()-timeStart );
                    db.close();
                    if (err){
        ...
        self.connect()
            .then(function (db){
                ...
                col.find(query, {skip: skip, limit:count}).toArray(function (err, items){
                    util.log('log;', 'Mongo.loadRanks query: db.users.find(', query, ') time: ', Date.now()-timeStart );
                    db.close();
                    if (err){
            ...
            .then(function (db){
                ...
                col.find(query, {"sort": [[column, order]], skip: skip, limit:count}).toArray(function (err, allUsers){
                    if (err){
                        ...
                        util.log('log;', 'Mongo.loadRating query: db.users.find(', query, ').sort({"', column,'" : 1}) time: ', Date.now()-timeStart );
                        query['userId'] = userId;
        ...
        self.connect()
            .then(function (db){
                ...
                col.find(query, { "sort": [['timeEnd', 'desc']], limit:count, skip: offset}).toArray(function (err, items){
                    ...
                    util.log('log;', 'Mongo.loadHistory query: db.history.find(', query, ').sort({timeEnd : -1}) time: ', Date.now()-timeStart );
                    res(items);
                });
            })
            .catch(function(err){
        ...
        self.connect()
            .then(function (db){
                ...
                col.findOne({_id: new ObjectId(gameId)}, {}, function (err, item){
                    db.close();
                    if (err){
                        util.log('err;', 'Mongo.loadGame', err);
                        rej(err);
                        return;
                    }
                    res(item);
...
Mongo.prototype.createIndexes = function(){
    var self = this;
        util.log('log;', 'Mongo.createIndexes', 'start');
        var indexPromises = [];
            ...
            for (var j = 0; j < self.collections[i].indexes.length; j++){
                ...
                if (index.mode){    // generate fields for each game mode
                    for (var n = 0; n < self.server.modes.length; n++){
                        ...
                        util.log('log;', 'Mongo.createIndexes', collection, mode+'_'+index.name, 'fields:', fields, 'unique:', !!index.unique);
                        indexPromises.push(self.createIndexNotExists(collection, mode+'_'+index.name, fields, !!index.unique));
                    }
                } else {
                    util.log('log;', 'Mongo.createIndexes', collection, index.name, 'fields:', index.fields, 'unique:', !!index.unique);
        ...
        self.connect()
            .then(function (db){
                ...
                col.indexExists(name, function (err, result){
                    if (err){
                        util.log('err;', 'Mongo.createIndexNotExists', err);
                        db.close();
                        res(null);
                        return;
                    }
                    if (result){
                        util.log('log;', 'Mongo.createIndexNotExists', name, 'exists');
                        db.close();
                        res(name);
                    } else {
                        util.log('log;', 'Mongo.createIndexNotExists', 'creating index', name);
                        col.createIndex(fields, {name: name, w: 1, unique: unique, dropDups: unique, background: true}, function(err, index){
                            db.close();
                            if (err){
                                util.log('err;', 'Mongo.createIndexNotExists', err);
                            }
                            util.log('log;', 'Mongo.createIndexNotExists', 'creating index', index, index?'complete':'failed');
                            res(index);
                        });
                    }
                });
