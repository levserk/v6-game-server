...
Mongo.prototype.init = function(callback){
    ...
    MongoClient.connect(this.url, function(err, db) {
        ...
        util.log('log;', 'Mongo', "Success connected to mongo ", self.url);
        // create collections
        db.collection('users');
        ...
        col.insertOne({time: key},function(err, resultInsert){
            util.log('log', 'Mongo test, insert', err );
            if (!resultInsert) {
                throw new Error('Mongo write test failed!');
            }
            if (resultInsert.insertedCount == 1){
                col.findOne({time: key}, function(err2, resultFind){
                    db.close();
                    util.log('log', 'Mongo test, find', err2, resultFind);
                    util.log('log', 'Mongo test ', err2, resultFind?'complete':'failed');
