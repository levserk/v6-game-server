        ...
        self.connect()
            .then(function (db){
                ...
                col.find(query, { "sort": [['time','desc']], limit:count}).toArray(function (err, items){
                    logger.log('Mongo.loadRanks query: db.messages.find(', query, ').sort({time: -1}) time: ', Date.now()-timeStart , 4);
                    db.close();
                    if (err){
        ...
        self.connect()
            .then(function (db){
                ...
                col.find(query, {skip: skip, limit:count}).toArray(function (err, items){
                    logger.log('Mongo.loadRanks query: db.users.find(', query, ') time: ', Date.now()-timeStart, 4);
                    db.close();
                    if (err){
            ...
            .then(function (db){
                ...
                col.find(query, {"sort": [[column, order]], skip: skip, limit:count}).toArray(function (err, allUsers){
                    if (err){
                        ...
                        logger.log('Mongo.loadRating query: db.users.find(', query, ').sort({"', column,'" : 1}) time: ', Date.now()-timeStart, 4);
                        query['userId'] = userId;
                        col.findOne(query, {}, function (err, infoUser){
                            db.close();
                            if (err || !infoUser){
                                logger.err('Mongo.loadRating user row', err, infoUser, err?1:3);
                            }
                            res({
                                allUsers: allUsers,
                                infoUser: infoUser,
        ...
        self.connect()
            .then(function (db){
                ...
                col.find(query, { "sort": [['timeEnd', 'desc']], limit:count, skip: offset}).toArray(function (err, items){
                    ...
                    logger.log('Mongo.loadHistory query: db.history.find(', query, ').sort({timeEnd : -1}) time: ', Date.now()-timeStart, 4);
                    res(items);
                });
            })
            .catch(function(err){
