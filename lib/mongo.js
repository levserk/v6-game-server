...
Mongo.prototype.getUserOptions = function (userId) {
    var self = this;
    return new Promise(function(res, rej){
        self.connect()
            .then(function (db){
                var col = db.collection('options');
                col.findOne({userId: userId},{}, function (err, item){
                    db.close();
                    if (err){
                        util.log('err;', 'Mongo.getUserOptions', err);
                    }
                    res(item?item.options:null);
                });
            })
            .catch(function(err){
                rej(err);
...
Mongo.prototype.saveUserOptions = function (userId, options){
    var self = this;
    return new Promise(function(res, rej){
        self.connect()
            .then(function (db){
                var col = db.collection('options');
                col.updateOne({userId: userId}, {userId: userId, options: options}, {upsert:true, w: 1}, function (err, result){
                    db.close();
                    if (err){
                        util.log('err;', 'Mongo.saveUserOptions error', err);
                        rej(err);
                        return;
                    }
                    util.log('log;', 'Mongo.saveUserOptions user updated', userId,
                        result.matchedCount,
                        result.modifiedCount,
                        result.upsertedId,
                        result.upsertedCount);
