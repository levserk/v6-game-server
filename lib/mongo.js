...
Mongo.prototype.init = function(callback){
    MongoClient.connect(this.url, function(err, db) {
        ...
        var key = Date.now();
        col.insertOne({key: key},function(err, result){
            util.log('log', 'Mongo test, insert', err, result.insertedCount);
            if (result.insertedCount == 1){
                col.findOne({key: key}, function(err2, result2){
                    db.close();
                    util.log('log', 'Mongo test, find', err2, result2);
