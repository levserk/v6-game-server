    ...
    return new Promise(function(res, rej){
        self.connect()
            .then(function (db){
                ...
                var query = {};
                query[mode] = { '$exists': true };
                col.find(query, {"sort": [[mode+'.'+'ratingElo', 'desc']], skip: skip, limit:count}).toArray(function (err, allUsers){
                    if (err){
                        ...
                        query['userId'] = userId;
                        col.find(query).toArray(function (err, infoUser){
                            db.close();
                            if (err){
                                util.log('err;', 'Mongo.loadRating user row', err);
                            }
                            if (!infoUser || infoUser.length != 1)
                                infoUser = null;
                            else infoUser = infoUser[0];
...
Mongo.prototype.loadGame = function(gameId){
    ...
    return new Promise(function(res, rej){
        self.connect()
            .then(function (db){
                var col = db.collection('games');
                col.find({_id: new ObjectId(gameId)}).toArray(function (err, items){
                    db.close();
