        ...
        self.connect()
            .then(function (db){
                ...
                col.find(query, { "sort": [['time','desc']], limit:count}).toArray(function (err, items){
                    logger.log('Mongo.loadRanks query: db.messages.find(', query, ').sort({time: -1}) time: ', Date.now()-timeStart , 3);
                    db.close();
                    if (err){
...
Mongo.prototype.loadRating = function(userId, mode, count, skip, column, order, filter){
    var self = this, timeStart = Date.now();
    ...
    return new Promise(function(res, rej){
        self.connect()
            .then(function (db){
                ...
                if (filter) query['userData'] = {$regex: '"'+filter, $options: 'i'};
