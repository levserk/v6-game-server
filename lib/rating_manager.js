...
RatingManager.prototype.onMessage = function(message, type){
    ...
    switch (type) {
        case 'ratings': // load history
            message.data.mode = message.data.mode || this.server.modes[0];
            this.server.storage.getRatings(message.sender.userId, message.data)
...
RatingManager.prototype.computeNewRatings = function (room, result, callback){
    util.log('log;', 'RatingManager.computeNewRatings', room.id);
