...
Server.prototype.init = function(){
    util.log('log;', 'GameServer init', this.version);
    ...
    this.router.on('socket_connection', function(socket){
        util.log("new socket_connection", socket.id, socket.cookie._userId);
        var user = new User(socket);
        var signIsRight = false;
        if (typeof self.engine.checkSign == "function") {
            signIsRight = self.engine.checkSign(user);
        } else {
            signIsRight = self.defaultEngine.checkSign(user);
        }
        if (!signIsRight){
            util.log('err;', 'Server', 'User sign is wrong', user.userId, user.userName, user.sign);
            self.onLoginError(user, 'wrong sign');
            return;
        }
