...
module.exports = function (opts, engine) {
    ...
    opts.resetTimerEveryTurn = opts.resetTimerEveryTurn || defaultConf.resetTimerEveryTurn;
...
Server.prototype.init = function(){
    ...
    this.router.on('socket_disconnected', function(socket){
        ...
        if (!user){ // TODO: do not log error on user relogin
            if (!socket.reconnectUser)
                logger.warn('Server','on socket_disconnected', 'user not found! socket: ', socket.id, socket.userId, 1);
            return;
        }
    ...
    this.router.on('socket_timeout', function(socket){
        ...
        var user = self.getUserById(socket.id);
        if (!user){
            logger.warn('Server', 'on socket_timeout', 'user not found! socket:', socket.id, socket.userId, 1);
            return;
        }
        user.isConnected = false;
        user.isTimeout = true;
...
Server.prototype.onUserChanged = function(user, data){
    ...
    if (now - user.lastTimeSendInfo > 1000){ // send user info once a second
        this.router.send({module:"server", type: "user_changed", sender:user, target:this.game, data:user.getInfo()});
...
Server.prototype.getUserById = function(id){
    ...
    logger.warn("Server.getUserById", "user not exists in userlist", id, 1);
    return null;
};
