...
Server.prototype.init = function(){
    ...
    this.router.on('socket_disconnected', function(socket){
        ...
        if (!user){ // TODO: do not log error on user relogin
            logger.warn('Server','on socket_disconnected', 'disconnected user not found! socket.id: ', socket.id, socket.cookie.userId, 3);
            return;
        }
    ...
    this.router.on('socket_timeout', function(socket){
        ...
        var user = self.getUserById(socket.id);
        if (!user){
            logger.warn('Server', 'on socket_timeout', 'disconnected timeout user not found! ', socket.id, socket.cookie.userId, 3);
            return;
        }
        user.isConnected = false;
        user.isTimeout = true;
...
Server.prototype.onUserChanged = function(user){
...
Server.prototype.onMessage = function(message, type){
    switch (type) {
        ...
        case 'settings': // player ready to play
            ...
            break;
        case 'changed':
            this.onUserChanged(message.sender);
            break;
    }
};
