...
function StorageInterface(server, callback){
    ...
    this.mongo.init(function(){
        this.mongo.createIndexes()
            .then(function(){
            util.log('log;', 'StorageInterface creating indexes complete');
            var promises = [];
            ...
            .catch(function(){
                util.log('err;', 'StorageInterface creating indexes failed');
                process.exit(1);
            });
    }.bind(this));
}
...
StorageInterface.prototype.getUserData = function(user){
    util.log('log;', 'StorageInterface.getUserData', user.userId);
    var self = this;
...
StorageInterface.prototype.deleteMessage = function(id){
    ...
    // TODO: delete message from cache
};
...
StorageInterface.prototype.loadRanks = function(mode) {
    ...
    function loadOrFinish(load){
        return load.then(function (obj) {
            ...
            util.log('log;', 'StorageInterface.loadRanks', 'insert redis ranks ',items.length, 'time:', Date.now() - timeStart);
