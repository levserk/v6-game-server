...
function StorageInterface(server, callback){
    ...
    this.redis = redis.createClient(server.conf.redis.port, server.conf.redis.host);
...
StorageInterface.prototype.popUser = function(user){
    ...
    logger.err("StorageInterface.popUser", "user not exists in userlist", user.userId, 2);
    return false;
};
...
StorageInterface.prototype.popRoom = function(room){
    ...
    logger.err("StorageInterface.popRoom", "room not exists in roomsInfo id:", id, 1);
};
...
StorageInterface.prototype.loadUserRank = function (userData, mode){
    return new Promise(function (res, rej) {
        ...
        this.redis.zrevrank(key, userData.userId, function(err, result) {
            logger.log('StorageInterface.loadUserRank before', userData[mode].rank, 4);
            if (err || result === null) {
                if (err) logger.warn('StorageInterface.loadUserRating get rank failed ', err, userData.userId, 3);
            } else {
                userData[mode].rank = result + 1;
            }
            logger.log('StorageInterface.loadUserRank after', userData[mode].rank, 4);
            res(userData)
        });
    }.bind(this));
};
