...
StorageInterface.prototype.getUserData = function(user){
    var self = this;
    var pool = this.mysqlPool, test = this.server.conf.mode == 'debug';
...
StorageInterface.prototype.updateRatings = function(mode) {
    ...
    for (i = 0; i < this.allUsers.length; i++)
        if (this.allUsers[i][mode]['games']>0)
    ...
    for (i = 0; i < users.length; i++){
        ...
        user = this.getUser(users[i].userId);
        if (user) {
            util.log('log;', 'StorageInterface.updateRatings', user[mode]['rank'], i+1);
            user[mode]['rank'] = i+1;
        }
    }
};
...
StorageInterface.prototype.getRatings = function(user, params){
    ...
    for (var i = 0; i < this.allUsers.length; i++) {
        ...
        if (this.allUsers[i].userId == user.userId) infoUser = this.allUsers[i];
    }
