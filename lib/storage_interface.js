...
StorageInterface.prototype.getUserData = function(user){
    ...
    return new Promise(function(res, rej){
        ...
        self.mongo.getUserData(user.userId)
            ...
            .then(self.loadUserName.bind(self))
            .then(function(userData){
                self.mongo.getUserOptions(userData.userId)
...
StorageInterface.prototype.loadUserName = function(userData){
  var self = this, test = this.server.conf.mode == 'debug';
    return new Promise(function(res, rej){
        if (!test) {
            var pool = self.mysqlPool;
            pool.query('SELECT u.userId, u.username ' +
                ' FROM kosynka_users AS u ' +
                " WHERE u.userId = ? ", [userData.userId],
                function(err, rows){
                    if (err) {
                        rej(err);
                        return;
                    ...
                    else rej('user not reg! userId: '+userData.userId + ' sessionId: ' + userData.sessionId);
                });
            return;
        }

        userData.userName = "us_"+userData.userId;
        if (!userData.userId) rej('user not reg! userId: '+userData.userId + ' sessionId: ' + userData.sessionId);
        else res(userData);
    });
};


StorageInterface.prototype.saveUserOptions = function(user, options){
    user.options = options;
    this.mongo.saveUserOptions(user.userId, options);
};
