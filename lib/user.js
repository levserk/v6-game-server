...
User.prototype.leaveRoom = function(){
    ...
    if (this.isConnected) { // closed socket already leave room
        logger.log('leaving room ', this.currentRoom.id, this.userId, 3);
        // TODO socket leave already leaved room
        try {
            this.socket.leaveRoom(this.currentRoom.id);
        } catch(e){
            logger.err('leaving room already leaved room', this.currentRoom.id, this.userId, e, 1);
        }
    }
    this.currentRoom = null;
};
